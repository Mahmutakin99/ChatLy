...
struct Service {
    ...
    @discardableResult
    static func fetchLastUsers(completion: @escaping([LastUser])->Void) -> ListenerRegistration?{
    ...
    static func fetchMessages(user: User, completion: @escaping([Message])->Void) -> ListenerRegistration {
        guard let currentUid = Auth.auth().currentUser?.uid else { fatalError("Current user UID not found") }
        ...
        let listener = query.addSnapshotListener { snapshot, error in
            ...
            let messages = snapshot?.documents.map { Message(data: $0.data()) } ?? []
            completion(messages)
        }
        return listener
    }
